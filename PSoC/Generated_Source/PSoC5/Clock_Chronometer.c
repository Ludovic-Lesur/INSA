...
/*******************************************************************************
* Function Name: Clock_Chronometer_Start
********************************************************************************
*
* Summary:
...
void Clock_Chronometer_Start(void) 
{
    ...
    Clock_Chronometer_CLKEN |= Clock_Chronometer_CLKEN_MASK;
	Clock_Chronometer_CLKSTBY |= Clock_Chronometer_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Chronometer_Stop
********************************************************************************
*
* Summary:
...
void Clock_Chronometer_Stop(void) 
{
    ...
    Clock_Chronometer_CLKEN &= (uint8)(~Clock_Chronometer_CLKEN_MASK);
	Clock_Chronometer_CLKSTBY &= (uint8)(~Clock_Chronometer_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Chronometer_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Chronometer_CLKEN & Clock_Chronometer_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Chronometer_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Chronometer_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Chronometer__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Chronometer_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Chronometer_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Chronometer_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Chronometer_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Chronometer_GetSourceRegister();
    uint16 oldDivider = Clock_Chronometer_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Chronometer_CLKEN & Clock_Chronometer_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clock_Chronometer_DIV_PTR, clkDivider);
                Clock_Chronometer_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clock_Chronometer_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clock_Chronometer_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Chronometer_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Clock_Chronometer_DIV_PTR, clkDivider);
				Clock_Chronometer_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Clock_Chronometer_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Chronometer_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Chronometer_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Chronometer_MOD_SRC |= modeBitMask & (uint8)Clock_Chronometer_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Chronometer_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Chronometer_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Chronometer_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Chronometer_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Chronometer_MOD_SRC & (uint8)(Clock_Chronometer_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Chronometer_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Chronometer_GetDividerRegister();
    uint8 oldSrc = Clock_Chronometer_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Chronometer_MOD_SRC |= CYCLK_SSS;
        Clock_Chronometer_MOD_SRC =
            (Clock_Chronometer_MOD_SRC & (uint8)(~Clock_Chronometer_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Chronometer_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Chronometer_MOD_SRC & Clock_Chronometer_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Chronometer_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Chronometer_PHASE = clkPhase & Clock_Chronometer_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Chronometer_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Chronometer_PHASE & Clock_Chronometer_PHASE_MASK;
}
